...
- **廣泛的 RAW 格式支援。** 十種 RAW 容器格式皆可在 GPU 上完整解碼至全解析度，其中針對素以
  ...
  核心，而非退回 CPU 處理；另外三種較舊的容器格式（CR2、IIQ、MRW）目前僅能透過內嵌預覽圖
  瀏覽，詳見 [RAW 格式支援與解碼路由](#raw-格式支援與解碼路由)。同時 Halcyon 也修正了 libraw
...
（任何以點開頭的名稱，包括 macOS 在某些記憶卡上散落的 AppleDouble sidecar 檔）一律跳過，只有
支援格式的檔案才會顯示。
...
Halcyon 會把它們分組：一張 RAW 與其同名 JPG（以及任何隱藏 sidecar 檔）合成側邊欄裡的**一個項目**，
...
回來時會俐落地吸附回原尺寸，不會讓畫面停在偏移的位置。切換照片時縮放層級會維持不變，
從一張換到下一張不會把它重置。
...
星號與垃圾桶標記都完好無缺。標記不只存在記憶體裡，你一做出標記就會立刻寫入磁碟，你當時
所在的照片也會被記住。
...
一起搬動（macOS 產生的任何隱藏 sidecar 檔也會一併清除，不會殘留在目的地）。若目的地已有同名檔案，
...
回收模式是最保險的選項：它把要丟照片的每一個檔案——包括其 RAW 姊妹檔與任何隱藏 sidecar 檔——移到
...
RAW 檔、它的 JPG 對應檔，以及任何隱藏的 sidecar 檔，都會一起改成相同的新基底檔名，所以 RAW+JPG
配對絕不會被拆散。
...
Halcyon 以「每張照片一次」讀取 EXIF，涵蓋 RAW、JPG 對應檔與 sidecar 檔整組；有 JPG 對應檔時優先
...
在還沒支援完整解碼的平台上，如果一張 RAW 檔剛好沒有內建預覽，會暫時顯示成無法預覽，這是平台功能還沒補齊，照片本身沒有損壞。
...
上實際需要 macOS 15、Intel 上需要 macOS 14。** 這是兩件各自獨立的事實：
應用程式本身宣告的最低版本沒有改變，較高的版本數字描述的只是隨附解碼器函式庫本身
載入時的需求。這個門檻直接從各函式庫自己的版本 load command 量出來：在 Apple
Silicon 上，真正卡住下限的是隨附 OpenMP runtime 要求的 macOS 15；
...
建置產出物一律落在根目錄的 `build/` 底下。`android/`、`ios/`、`macos/`、`web/`、`windows/` 與 `linux/` 是原始碼與設定，會留在版本控制中。
...
`flutter analyze` 回報零問題是硬性閘門，只要它報出任何問題，工作就還沒完成。注意靜態分析的範圍涵蓋 `lib/`、`test/` **以及** `tool/`，所以只掃過 `lib/` 與 `test/` 的符號重新命名，仍然會讓這道閘門過不了。
