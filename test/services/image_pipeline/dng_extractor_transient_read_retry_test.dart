...
/// TC-540 — a TRANSIENT read failure must not be reported as "no embedded
/// preview".
///
...
void main() {
  ...
  test('TC-540a: a THROWN read error is retried, not reported as "no preview"',
      () async {
    final run = await probeWithFaults(
      await writePreviewBearingDng(),
      failFirstOpens: 1,
  ...
  test('TC-540d: a SHORT read (no exception) on the preview strip is retried',
      () async {
  ...
  test('TC-540b: probeContent measurement survives a transient read failure',
      () async {
  ...
  test('TC-540c: a genuinely preview-less container is NOT retried', () async {
