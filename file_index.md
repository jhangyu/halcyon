...
│   │   └── support/
│   │       └── synthetic_dng.dart  # 測試用合成 DNG bytes 產生器（非測試檔本身）
...
| `dartImageLoad` | `lib/services/dart_image_loader.dart` | 純 Dart 影像位元組生產實作，取代已刪除的 native thumbnail MethodChannel；`ImageRequestPurpose` 決定目標長邊（sidebarThumbnail 200px／preview 2800px／export 2048px） |
| `PhotoSource` / `PrefetchScheduler` | `lib/services/photo_source.dart`、`lib/services/prefetch_scheduler.dart` | `PhotoSource.probe` 從內容量測來源成本；排程常數控制 expensive source 啟動半徑與 tier-2 precache 半徑 |
...
| 影像請求長邊（`ImageRequestPurpose`） | sidebarThumbnail 200px／preview 2800px／export 2048px（`lib/services/image_source_types.dart`）|
